...
def solve_convex_subproblem(J_grad, J_m_base, u_ref, trust_radius, trust_norm, w_max, a_max, a_min):
    ...
    for k in range(N):
        constraints += [
            u_normlized[k, 0] <= 1,  # angular velocity constraint (normalized) 
            u_normlized[k, 0] >= 0,
...
def SCP(
    ...
    while True:
        if eta >= eta_max:
            print("Penalty eta reached maximum value. Stopping outer loop.")
            break
        elif outer_iter >= max_outer:
            print("Maximum outer iterations reached. Stopping outer loop.")
            break
        outer_iter += 1
        ...
        while inner_iter < max_inner:
            ...
            try:
                ...
                if act_dec < 0:
                    d = min(d * shrink, d_max)
                    print("Actual decrease negative. Shrinking trust region and retrying., iter: {}, act_dec: {:.6f}, d: {:.6f}".format(inner_iter, act_dec, d))
                    # trust_region_shrunk = True
                    if d <= d_min:
                ...
                if denom < 0:
                    print("Predicted decrease negative. This should not happen. Setting denom to small positive value. iter: {}, denom: {:.6f}".format(inner_iter, denom))
                ...
                # accept step
                if np.linalg.norm(u_star - u_bar) < tau_conv:
                    u_bar = u_star
                    print(f"Inner loop converged at iter {inner_iter}. by (u_star-u_bar) norm")
                    break
                elif abs(J_m_grad.flatten() @ (u_star - u_bar).flatten()) < tau_f:
                    u_bar = u_star
                    print(f"Inner loop converged (first-order) at iter {inner_iter}.")
                    break
                ...
                if rho < rho_reject:
                    ...
                    # u_bar = u_star
                    d = max(d * shrink, d_min)
                    print("rho is too small. By rho_reject, Shrinking trust region and retrying., iter: {}, rho: {:.4f}, d: {:.6f}".format(inner_iter, rho, d))
                    if d <= d_min:
                        print("Trust region radius too small. Stopping inner loop. iter: {}".format(inner_iter))
                        break
                    continue
                    ...
                    u_bar = u_star
                    print(f"Step accepted and trust region expanded., iter: {inner_iter}, rho: {rho:.4f}, act_dec: {act_dec:.6f}, pred_dec: {denom:.6f}, d: {d:.4f}")
                else:
                    print(f"Step accepted without expand., iter: {inner_iter}, rho: {rho:.4f}, act_dec: {act_dec:.6f}, pred_dec: {denom:.6f}, d: {d:.4f}")
                    u_bar = u_star
            # except Exception as e:
            #     print("Exception in inner SCP loop at iter {}: {}".format(inner_iter, e))
            finally:
                # print(f"  Inner iter {inner_iter}: J_m = {Jm_star:.4f}, act_dec = {act_dec:.6f}, pred_dec = {denom:.6f}, rho = {rho:.4f}, d = {d:.4f}")
                pass
