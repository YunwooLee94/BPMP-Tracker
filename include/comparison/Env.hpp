...
class Env {
 ...
 public:
  ...
  inline bool findVisiblePath(const Eigen::Vector3i& start_idx,
    ...
    // NOTE 6-connected graph -> 4-connected in xy plane //승우
    for (int i = 0; i < 2; ++i) {
    // for (int i = 0; i < 2; ++i) {
    ...
    if (curPtr == nullptr) {
      std::cout << "[env] out of memory when visiting start node!" << std::endl;
      return false;
    }
    ...
    while (visited_nodes_.size() < MAX_MEMORY) {
      ...
      for (const auto& neighbor : neighbors) {
        ...
        if (neighborPtr == nullptr) {
          std::cout << "[env] out of memory when visiting neighbors!" << std::endl;
          open_set = std::priority_queue<NodePtr, std::vector<NodePtr>, NodeComparator>();
          break;
        }
  ...
  inline bool astar_search(const Eigen::Vector3i& start_idx,
    ...
    NodePtr curPtr = visit(start_idx);
    if (curPtr == nullptr) {
      std::cout << "[astar search] out of memory when visiting start node!" << std::endl;
      return false;
    }
    // NOTE we should permit the start pos invalid! (for corridor generation)
    if (!curPtr->valid) {
    ...
    while (visited_nodes_.size() < MAX_MEMORY) {
      for (const auto& neighbor : neighbors) {
        ...
        NodePtr neighborPtr = visit(neighbor_idx);
        if (neighborPtr == nullptr) {
          std::cout << "[astar search] out of memory when visiting neighbors!" << std::endl;
          open_set = std::priority_queue<NodePtr, std::vector<NodePtr>, NodeComparator>();
          break;
        }
        if (neighborPtr->state == CLOSE) {
  ...
  inline bool short_astar(const Eigen::Vector3d& start_p,
    ...
    NodePtr curPtr = visit(start_idx);
    if (curPtr == nullptr) {
      std::cout << "[short astar] out of memory when visiting start node!" << std::endl;
      return false;
    }
    // NOTE we should permit the start pos invalid! (for corridor generation)
    if (!curPtr->valid) {
    ...
    while (visited_nodes_.size() < MAX_MEMORY) {
      for (const auto& neighbor : neighbors) {
        ...
        NodePtr neighborPtr = visit(neighbor_idx);
        if (neighborPtr == nullptr) {
          std::cout << "[short astar] out of memory when visiting neighbors!" << std::endl;
          open_set = std::priority_queue<NodePtr, std::vector<NodePtr>, NodeComparator>();
          break;
        }
        if (neighborPtr->state == CLOSE) {
