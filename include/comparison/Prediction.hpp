...
struct Predict {
 private:
  ...
  inline bool isValid(const Eigen::Vector3d& p, const Eigen::Vector3d& v) const {
    // // std::cout << "[prediction] checking state: p=" << p.transpose() << ", v=" << v.transpose() << std::endl;
    // if (map.isOccupied(p)){ 
    //   // std::cout << "[prediction] position occupied!" << std::endl;
    //   return false; 
    // }
    // else if(v.norm() >= vmax){
    //   // std::cout << "[prediction] velocity too high!" << std::endl;
    //   return false;
 ...
 public:
  inline Predict(ros::NodeHandle& nh) {
    ...
    std::cout << "[prediction] params: pre_dur=" << pre_dur << ", dt=" << dt
              << ", rho_a=" << rho_a << ", vmax=" << vmax << std::endl;
    ...
    while (curPtr->t < pre_dur) {
      for (input.x() = -3; input.x() <= 3; input.x() += 3)
        for (input.y() = -3; input.y() <= 3; input.y() += 3) {
          ...
          if (!isValid(p, v)) {
            // std::cout << "[prediction] invalid state!" << std::endl;
            continue;
          }
